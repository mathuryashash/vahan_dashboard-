"""Standalone entrypoint for the Fuel x Vehicle Class cross-tab scrape.
Mirrors run_maker_category_scrape.py exactly -- see
docs/superpowers/specs/2026-08-25-maker-category-crosstab-design.md.

Usage: python -m scraper.run_fuel_category_scrape [--year YYYY] [--force]
"""
import argparse
import asyncio
import logging
...
from app.services.scraper_service import persist_fuel_category_batch, _state_code_lookup
from scraper.vahan_scraper import scrape_all_india_fuel_category

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger("run_fuel_category_scrape")


async def _already_done_rtos(db, year: int) -> dict[str, frozenset[str]]:
    query = select(FuelCategoryTotal.state_name, FuelCategoryTotal.rto_code).where(
...
async def main(year: int, force: bool = False) -> None:
    logger.info(
        "Starting Fuel x Vehicle Class scrape (year=%s, force=%s) at %s",
    ...
    async with AsyncSessionLocal() as db:
        ...
        rto_count = 0
        async for item in scrape_all_india_fuel_category(year=year, skip_rtos=skip_rtos):
            ...
            if code is None:
                ...
                continue
            await persist_fuel_category_batch(db, batch, state_code=code, year=year)
            await db.commit()
            rto_count += 1
            if rto_count % 25 == 0:
                logger.info("Scraped %d RTOs so far...", rto_count)

    logger.info("Fuel x Vehicle Class scrape complete (year=%s). %d RTOs processed.", year, rto_count)
