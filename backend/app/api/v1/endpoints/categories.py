...
async def get_top_makers(
    ...
    if vehicle_class or vehicle_category or commercial_tier:
        ...
        gaps = await makers_with_coverage_gaps(
            db, MakerCategoryTotal, year, [r["maker"] for r in response],
            state=state, rto_code=user_rto,
        )
        for row in response:
            row["partial"] = row["maker"] in gaps
    ...
    # NOT coverage-checked, deliberately. registrations has the same
    # per-maker gaps as the crosstab (2024 and 2025 here, both scraped by the
    # paginating parser that silently dropped pages, since replaced), but no
    # index on this table leads with `maker` -- the crosstab answers in ~2s
    # via idx_mct_year_maker_state_count, while the same question here is a
    # 16-23s sequential scan, measured. That cannot sit on a page load, and
    # adding the index means locking an 8.3GB table.
    # The real fix for those two years is re-scraping them; see
    # makers_with_coverage_gaps' docstring.
    _top_makers_cache.set(cache_key, response)
    return response
...
async def get_maker_category_breakdown(
    ...
    # Only when ranking makers: mark the ones whose figure for this year is
    # built on a fraction of the RTOs they normally cover, so the chart can
    # say so instead of presenting a scraping gap as a collapse in sales.
    if not maker:
        gaps = await makers_with_coverage_gaps(
            db, MakerCategoryTotal, year, [r[key_name] for r in response],
            state=state, rto_code=user_rto,
        )
        for row in response:
